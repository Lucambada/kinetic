...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'REGISTRATION FAILED: UNKNOWN ERROR');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="rounded-2xl border border-white/10 bg-surface-container-low p-8 shadow-2xl backdrop-blur-xl">
          {success ? (
            <div className="text-center space-y-4">
              ...
              <h2 className="text-xl font-bold text-green-500 uppercase tracking-widest">Registration Successful</h2>
              <p className="text-xs text-on-surface-variant uppercase tracking-widest">Redirecting to terminal login...</p>
            </div>
          ) : (
